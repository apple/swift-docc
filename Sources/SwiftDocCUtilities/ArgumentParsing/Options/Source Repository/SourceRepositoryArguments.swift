...
public struct SourceRepositoryArguments: ParsableArguments {
    ...
    @Option(
        help: ArgumentHelp(
            "The root path on disk of the repository's checkout.",
            discussion: "By default, this is set to the current working directory."
        )
    )
...
extension SourceRepository {
    init?(from arguments: SourceRepositoryArguments) throws {
        switch (arguments.sourceService, arguments.sourceServiceBaseURL) {
        case (nil, nil):
            return nil
        case (nil, _):
        ...
        case let (sourceService?, sourceServiceBaseURL?):
            ...
            let checkoutPath = arguments.checkoutPath ?? FileManager.default.currentDirectoryPath
