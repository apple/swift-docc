...
class ConvertSubcommandSourceRepositoryTests: XCTestCase {
    ...
    func testUsesCurrentDirectoryPathIfCheckoutPathIsNotSpecified() throws {
        try assertSourceRepositoryArguments(
            checkoutPath: nil,
            sourceService: "github",
            sourceServiceBaseURL: "example.com/path/to/base"
        ) { action in
            XCTAssertEqual(action.sourceRepository?.checkoutPath, FileManager.default.currentDirectoryPath)
        }
    }
    ...
    func testThrowsValidationErrorWhenSourcServiceIsInvalid() throws {
        XCTAssertThrowsError(
            try assertSourceRepositoryArguments(
                checkoutPath: nil,
